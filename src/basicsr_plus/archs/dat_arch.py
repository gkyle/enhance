...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, N, C = x.shape
        x2 = self.conv(self.norm(x2).transpose(1, 2).contiguous().view(
            B, C//2, H, W)).flatten(2).transpose(-1, -2).contiguous()
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def calculate_mask(self, H, W):
        ...
        ).view(-1, self.split_size[0], self.split_size[1], 1)  # nW, sw[0], sw[1], 1
        ...
        ).view(-1, self.split_size[1], self.split_size[0], 1)  # nW, sw[1], sw[0], 1
